(function() {
  ...
  module.exports = NodeCache = (function(superClass) {
    ...
    NodeCache.prototype.get = function(key, cb) {
      var _ret;
      if ((this.data[key] != null) && this._check(key, this.data[key], true)) {
        this.stats.hits++;
    ...
    NodeCache.prototype._check = function(key, data, hit) {
      var ttlMultiplicator = 1000, now = Date.now();
      if(hit == true && this.options.extendTTL == true) {
        if(data.t !== 0 && this.options.stdTTL !== 0) {
          data.t = now + (this.options.stdTTL * ttlMultiplicator);
          console.log("extend TTL");
          return true;
        }
      }
